...
class GitHubHooks:
    ...
    def deployment(self, headers, body):
        ...
        return 'The %s environment has been deployed.' \
            % (C.Bold(body['deployment']['environment']),)

    def deployment_status(self, headers, body):
        logging.warning('GitHubHooks.deployment')
        return 'The %s environment has been deployed with %s.' \
            % (C.Bold(body['deployment']['environment']),
    ...
    def gollum(self, headers, body):
        string = '%s updated the wiki. (%s)' \
                  % (C.Cyan(body['sender']['login']),
                     C.Blue(URLShortener.short(
                         body['repository']['html_url']+'/wiki'), False))
        for page in body['pages']:
            string += '\n%s %s %s. (%s)' \
                    % (C.Gray(page['sha'][:7]),  # Really 7 for pages sha ?
                       page['action'],
    ...
    def issue_comment(self, headers, body):
        return '%s commented issue %s. (%s)' \
                % (C.Cyan(body['comment']['user']['login']),
                   C.Gray('#'+str(body['issue']['number'])),
                   C.Blue(URLShortener.short(body['issue']['html_url']),
                          False))

    def member(self, headers, body):
        return '%s added %s as collaborator.' \
                % (C.Cyan(body['sender']['login']),
    ...
    def pull_request(self, headers, body):
        ...
        if body['action'] in ['assigned', 'unassigned']:
            string += '%s on ' % (C.Cyan(body['assignee']['login']), )
        string += 'pull request %s. (%s)' \
            % (C.Gray('#'+str(body['pull_request']['number'])),
               C.Blue(URLShortener.short(body['pull_request']['html_url']),
                      False))

    def pull_request_review_comment(self, headers, body):
        return '%s commented pull request %s. (%s)' \
                % (C.Cyan(body['comment']['user']['login']),
                   C.Gray('#'+str(body['pull_request']['number'])),
                   C.Blue(URLShortener.short(body['comment']['html_url']),
                          False))
