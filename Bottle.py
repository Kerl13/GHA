...
try:
    ...
    try:
        ...
        try:
            ...
            def json_dumps(data):
                raise ImportError("JSON support requires Python 2.6 or "
                                  "simplejson.")
            json_lds = json_dumps
...
try:
    ...
    def _stdout(x): sys.stdout.write(x)

    def _stderr(x): sys.stderr.write(x)
...
if py3k:
    ...
    def json_loads(s): json_lds(touni(s))

    def callable(x): hasattr(x, '__call__')
...
class Router(object):
    ...
    def add(self, rule, method, target, name=None):
        ...
        if (flatpat, method) in self._groups:
            ...
            self.dyna_routes[method][self._groups[flatpat, method]] \
                = whole_rule
        else:
...
class Bottle(object):
    ...
    def uninstall(self, plugin):
        ...
        remove all plugins with a matching ``name`` attribute or ``True`` to
        remove all plugins. Return the list of removed plugins.
        """
    ...
    def _cast(self, out, peek=None):
        ...
        if isinstance(first, HTTPResponse):
            ...
            def encoder(x): x.encode(response.charset)
...
class BaseRequest(object):
    ...
    def chunked(self):
        ...
        return 'chunked' in self.environ.get('HTTP_TRANSFER_ENCODING',
                                             '').lower()
    ...
    def urlparts(self):
        """ The :attr:`url` string as an :class:`urlparse.SplitResult` tuple.
            ...
            server. """
        env = self.environ
        http = env.get('HTTP_X_FORWARDED_PROTO') or env.get('wsgi.url_scheme',
                                                            'http')
    ...
    def path_shift(self, shift=1):
        ...
        self['SCRIPT_NAME'], self['PATH_INFO'] = path_shift(script, self.path,
                                                            shift)
...
class FileCheckerThread(threading.Thread):
    ...
    def run(self):
        ...
        def mtime(p): os.stat(p).st_mtime
        files = dict()
...
class BaseTemplate(object):
    ...
    def __init__(self, source=None, name=None, lookup=None, encoding='utf8',
        ...
        self.filename = source.filename if hasattr(source, 'filename') \
            else None
...
class SimpleTemplate(BaseTemplate):
    ...
    def prepare(self, escape_func=html_escape, noescape=False, syntax=None,
                **ka):
        self.cache = {}
        enc = self.encoding
...
ext = _ImportRedirect('bottle.ext' if __name__ == '__main__'
                      else __name__+".ext", 'bottle_%s').module
